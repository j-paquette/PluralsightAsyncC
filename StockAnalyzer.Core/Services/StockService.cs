...
namespace StockAnalyzer.Core.Services
{
    ...
    public class MockStockService : IStockService
    {
        public Task<IEnumerable<StockPrice>> 
            ///Just want to run the task asynchronously without using "await"
            GetStockPricesFor(string stockIdentifier, 
            CancellationToken cancellationToken)
        {
            ...
            //In this case, adding async and await would just add overhead and complexity we don't need
            //This allows us to represent a completed task with a result without having to run it asynchronoursly
